...
__global__ void sum_cuda(double* a, double *s, int width) {
  ...
  if (idx < width) {
    // Acumula a soma parcial do bloco diretamente em memória global (sem __shared__)
    atomicAdd(&s[blockIdx.x], a[idx]);
  }
}
...
int main()
{
  ...
  // zera as somas parciais no device (necessário para usar atomicAdd com corretude)
